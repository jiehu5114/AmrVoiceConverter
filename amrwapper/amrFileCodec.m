...
NSData * convertAndroid3GP2AMR(NSData *data) {
//http://android.amberfog.com/?p=181
    ...
    do {
        ...
        remainSize = (int)(size - boxSize);
    } while ((type != 0x6D646174) && (count < dataLenght)); // mdat section
    
    NSMutableData *amrData  = nil;
    
    if (count < dataLenght) {
        int rawAmrDataLength = remainSize;
...
NSData* DecodeAmrOr3GP2RawAmr(NSData* data) {
    NSData *rawAmrData  = data;
    ...
    {
		rawAmrData = convertAndroid3GP2AMR(data);
    }
    
    rfile = [rawAmrData bytes];
    if (strncmp(rfile, AMR_MAGIC_NUMBER, strlen(AMR_MAGIC_NUMBER)))
    {
        return nil;
    }
...
NSData* DecodeAMRToWAVE(NSData* data) {
    ...
    if (strncmp(rfile, AMR_MAGIC_NUMBER, strlen(AMR_MAGIC_NUMBER)))
    {
		data = convertAndroid3GP2AMR(data);
    }
